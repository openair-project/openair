...
find.time.interval <- function(dates) {
  ...
  id <- which.max(table(diff(as.numeric(unique(
    dates[order(dates)]
  )))))
...
selectByDate <- function(
  ...
  if (!missing(month)) {
    if (is.numeric(month)) {
      ...
      mydata <- subset(
        mydata,
        substr(tolower(format(date, "%B")), 1, 3) %in%
...
strip.fun <- function(results.grid, type, auto.text) {
  ...
  pol.name <- sapply(levels(factor(results.grid[[type[1]]])), function(x) {
  ...
  if (length(type) == 1) {
    ...
    pol.name <- sapply(levels(factor(results.grid[[type[2]]])), function(x) {
      quickText(x, auto.text)
    })
