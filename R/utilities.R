...
# Round values to the point in which they're no longer unique
round_safely <- function(x, n = 10, n_min = 0) {
  progress <- TRUE
  while (progress && n >= n_min) {
    new_x <- round(x, n)
    if (length(unique(new_x)) != length(unique(x))) {
      progress <- FALSE
    } else {
      x <- new_x
      n <- n - 1
