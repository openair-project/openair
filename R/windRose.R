...
#'
#' By default [windRose()] will plot a wind rose using "paddle" style segments
...
#'   are 45 and 10. Note that the width of the wind speed interval may need
...
#'   frequencies.
#' @param cols Colours to be used for plotting. Passed to [openColours()].
...
#' @param width For `paddle = TRUE`, the adjustment factor for width of wind
#'   speed intervals. For example, `width = 1.5` will make the paddle width 1.5
#'   times wider.
#' @param seg When `paddle = TRUE`, `seg` determines with width of the segments.
...
#'   annotations may be added by setting value to `c("annotation 1", "annotation
#'   2")`.
...
#'   (`xlab`, `ylab`, `main`) are passed to [lattice:xyplot()] via [quickText()]
...
windRose <- function(
  ...
  prepare.grid <- function(mydata) {
    ...
    if (all(is.na(mydata$x))) {
      ...
      levels(mydata$x) <- c(paste("Interval", seq_along(labs), sep = ""))
...
panel.wdprob <- function(dat, seg, angle, off.set) {
  ...
  lapply(seq_len(nrow(dat)), makeline, dat)
}
